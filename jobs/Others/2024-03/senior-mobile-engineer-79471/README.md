# [Senior Mobile Engineer](https://www.remotewlb.com/apply/senior-mobile-engineer-79471)  
### Calendly  
#### `💰 464255.0` `🌎 United States`  

### About the team & opportunity

What’s so great about working on Calendly’s Engineering team?

We make things possible for our customers through innovation.

Why do we need you? Well, we are looking for an engineer who will bring deep technical know-how, product focused mindset, and enthusiasm to our team. You will report to one of our Engineering Managers and will be responsible for building mobile features and the APIs that support them.

 **A day in the life of a Senior Mobile Engineer at Calendly**

You build and expand a mobile application and the growing service based ecosystem that supports it. Collaborating alongside other teams within Calendly to build in an API-first manner. You enjoy building all aspects of data driven mobile applications. Top candidates are proactive, stakeholder-minded, and empathetic. You and your team members impact every aspect of development from design, delivery and maintenance. You work closely with product managers to plan and implement complete solutions. You are comfortable working in numerous technologies such as **Swift, UIKit, SwiftUI,** **Ruby on Rails, Node.js, TypeScript, Docker, PostgreSQL, and more**.

On a typical day, **you** will be working on:

  * Advocating for strong foundations in technical architecture while maintaining high velocity
  * Working end-to-end to deliver features - from discovery, product design, technical design, coding, testing, shipping, and iteration based on collaborator feedback
  * Using your mobile development expertise to bridge the gap between the API and Mobile apps, balancing short and long term needs
  * Reviewing tech specs and pull requests from other specialists
  * Improving our engineering systems and processes; we operate an agile process with sprints and iterate continuously
  * Mentoring and coaching other team members

### What do we need from you?

  * 7+ years of Software Development experience at a SaaS or technology company
  * Proficiency in using RESTful APIs to integrate mobile applications with backend services
  * Expertise in the iOS SDK, Xcode and related development tools
  * Knowledge of design patterns such as MVC and MVVM
  * Deep understanding of UIKit and/or SwiftUI components
  * Experience with Rails is nice to have, but not required; we know the best engineers will learn languages as needed
  * Excitement to craft simple software solutions to sophisticated problems
  * Ability to craft clean, reliable and highly maintainable code
  * Authorized to work lawfully in the United States of America as Calendly does not engage in immigration sponsorship at this time

### What’s in it for you?

Ready to make a serious impact? Millions of people already rely on Calendly’s products, and we’re still in the midst of our growth curve — it’s a fantastic time to join us. Everything you’ll work on here will accelerate your career to the next level. If you want to learn, grow, and do the best work of your life alongside the best people you’ve ever worked with, then we hope you’ll consider allowing Calendly to be a part of your professional journey.

### Our Hiring Process:

We aim to provide an inclusive and equitable candidate experience to everyone who expresses interest in working at Calendly. To learn more about our hiring process, please visit our careers page at www.careers.calendly.com.

Once selected for an opportunity, the recruiter assigned to the role will keep you informed every step of the way. Have questions? Let your recruiter know! Want to share your experience? We are passionately committed to improving and building on our process, and we consider feedback a gift.

 ** _If you are an individual with a disability and would like to request a reasonable accommodation as part of the application or recruiting process, please contact us at recruiting@calendly.com ._**

 ** _Calendly is registered as an employer in many, but not all, states. If you are located in Alaska, Hawaii, Montana, North Dakota, South Dakota, Nebraska, Iowa, West Virginia, and Rhode Island, you will not be eligible for employment. Note that all individual roles will specify location eligibility._**

 ** _All candidates can find our Candidate Privacy Statement_** here

 ** _Candidates residing in California may visit our Notice at Collection for California Candidates here:_** Notice at Collection

Compensation is based on a variety of factors including but not limited to location, experience, and job-related skills. In addition, Calendly offers a wide range of best in class total rewards. This includes comprehensive employee benefits like healthcare, dental, vision, parental leave, 401(k) match, paid time off, and much more. At Calendly we believe exceptional performance deserves exceptional rewards! During the hiring process, we are committed to sharing details about the compensation range for the position, enabling you to make an informed decision.

Please note that the compensation details listed in role postings reflect the base salary only, and do not include bonus/commission, equity, or benefits.

Base Pay Range National - Salary$158,950—$215,050 USDBase Pay Range California - Salary$184,195—$249,205 USD  
## CLICK TO [APPLY](https://www.remotewlb.com/apply/senior-mobile-engineer-79471)

